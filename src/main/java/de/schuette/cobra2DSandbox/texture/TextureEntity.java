...
public class TextureEntity extends ImageModifierEntity {
	...
	/**
	 * Renders the texture by resizing with the entity's size. This method is used
	 * to render a modified image with the resize function.
	 *
	 * @param texture  The modified image to render resized.
	 * @param graphics The graphics from renderer.
	 * @param position The screen position given by the renderer.
	 */
	public void renderTexture(double degrees, final VolatileImage texture, final Graphics2D graphics,
			final Point position) {
		final Dimension imageSize = new Dimension(texture.getWidth(), texture.getHeight());

		if (this.size == null || imageSize.equals(this.size)) {
			RenderToolkit.renderTo(degrees, 0, position, graphics, texture);
		} else {
			RenderToolkit.renderTo(degrees, position, this.size, graphics, texture);
