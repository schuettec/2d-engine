...
public class ImageMath {
	 ...
	 * @return the output value
	 */
	public static float smoothPulse(final float a1, final float a2,
			final float b1, final float b2, float x) {
		if (x < a1 || x >= b2) {
			return 0;
		}
		if (x >= a2) {
	...
	public static float triangle(final float x) {
		final float r = ImageMath.mod(x, 1.0f);
	 ...
	 * @return the interpolated value
	 */
	public static int bilinearInterpolate(final float x, final float y,
			final int[] p) {
		float m0, m1;
	...
	public static float spline(float x, final int numKnots, final float[] knots) {
		...
		x = ImageMath.clamp(x, 0, 1) * numSpans;
		span = (int) x;
		if (span > numKnots - 4) {
			span = numKnots - 4;
		}
		...
		c3 = ImageMath.m00 * k0 + ImageMath.m01 * k1 + ImageMath.m02 * k2
				+ ImageMath.m03 * k3;
		c2 = ImageMath.m10 * k0 + ImageMath.m11 * k1 + ImageMath.m12 * k2
				+ ImageMath.m13 * k3;
		c1 = ImageMath.m20 * k0 + ImageMath.m21 * k1 + ImageMath.m22 * k2
				+ ImageMath.m23 * k3;
		c0 = ImageMath.m30 * k0 + ImageMath.m31 * k1 + ImageMath.m32 * k2
				+ ImageMath.m33 * k3;
	 ...
	 * @return the spline value
	 */
	public static float spline(final float x, final int numKnots,
			final int[] xknots, final int[] yknots) {
		int span;
		final int numSpans = numKnots - 3;
		float k0, k1, k2, k3;
		float c0, c1, c2, c3;
	 ...
	 * @return the spline value
	 */
	public static int colorSpline(final int x, final int numKnots,
			final int[] xknots, final int[] yknots) {
		int span;
		final int numSpans = numKnots - 3;
		float k0, k1, k2, k3;
	 ...
	 */
	public static void resample(final int[] source, final int[] dest,
			final int length, final int offset, final int stride,
			final float[] out) {
		int i, j;
		float sizfac;
		float inSegment;
